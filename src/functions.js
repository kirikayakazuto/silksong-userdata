...
export function AddGeoAndShellShardsNumber(jsonObj, geoNumber, shellShardsNumber) {

    let playerdata = jsonObj['playerData'];
    if(!playerdata) {
        console.log("没找到玩家数据, 请联系作者");
        return
    }
    playerdata['geo'] = parseInt(playerdata['geo']) + geoNumber;
    playerdata['ShellShards'] = parseInt(playerdata['ShellShards']) + shellShardsNumber;
}

export function PrintGeoAndShellShardsNumber(jsonObj) {
    let playerdata = jsonObj['playerData'];
    if(!playerdata) {
        console.log("没找到玩家数据, 请联系作者");
        return
    }
    console.log("当前念珠数量: ", playerdata['geo']);
    console.log("当前碎片数量: ", playerdata['ShellShards']);
}